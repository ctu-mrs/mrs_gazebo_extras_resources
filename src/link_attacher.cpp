...
void GazeboRosLinkAttacher::Load(physics::WorldPtr _world, sdf::ElementPtr /*_sdf*/) {
	...
	if (!ros::isInitialized()) {
		ROS_FATAL_STREAM(
				"[GazeboRosLinkAttacher]: A ROS node for Gazebo has not been initialized, unable to load plugin. " << "Load the Gazebo system plugin 'libgazebo_ros_api_plugin.so' in the gazebo_ros package)");
		return;
	}
...
bool GazeboRosLinkAttacher::do_attach(std::string model1, std::string link1, std::string model2, std::string link2, std::string joint_type) {
	...
	if (this->getJoint(model1, link1, model2, link2, j)) {
		ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Joint "<<model1<<"->" <<link1
				<<" to " <<model2<<"->" << link2<<" already existed, reusing it for attach.");
	...
	ROS_INFO_STREAM("[GazeboRosLinkAttacher]: Creating " << joint_type << " joint " <<model1<<"->" <<link1
			<<" to " <<model2<<"->" << link2<< " and attaching");
...
bool GazeboRosLinkAttacher::attach_callback(mrs_msgs::GazeboAttach::Request &req, mrs_msgs::GazeboAttach::Response &res) {
	ROS_INFO_STREAM(
			"[GazeboRosLinkAttacher]: Received request to attach model: '" << req.model_name_1 << "' using link: '" << req.link_name_1 << "' with model: '" << req.model_name_2 << "' using link: '" << req.link_name_2 << "'");
...
bool GazeboRosLinkAttacher::attach_typed_callback(mrs_msgs::GazeboAttachTyped::Request &req, mrs_msgs::GazeboAttachTyped::Response &res) {
	ROS_INFO_STREAM(
			"[GazeboRosLinkAttacher]: Received request to attach model: '" << req.model_name_1 << "' using link: '" << req.link_name_1 << "' with model: '" << req.model_name_2 << "' using link: '" << req.link_name_2 << "' and joint '" << req.joint_type << "'");
...
bool GazeboRosLinkAttacher::detach_callback(mrs_msgs::GazeboAttach::Request &req, mrs_msgs::GazeboAttach::Response &res) {
	ROS_INFO_STREAM(
			"[GazeboRosLinkAttacher]: Received request to detach model: '" << req.model_name_1 << "' using link: '" << req.link_name_1 << "' with model: '" << req.model_name_2 << "' using link: '" << req.link_name_2 << "'");
...
bool GazeboRosLinkAttacher::apply_force_callback(mrs_msgs::GazeboApplyForce::Request &req, mrs_msgs::GazeboApplyForce::Response &res) {
	ROS_INFO_THROTTLE(1.0, "[GazeboRosLinkAttacher]: Received request to continuously apply force [%.2fN, %.2fN, %.2fN] to model: '%s'", req.force[0], req.force[1],
			req.force[2], req.model_name.c_str());
